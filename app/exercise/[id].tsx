...
import { ChevronLeft, Copy, Minus, Pencil, Plus, Trash2, TrendingUp, X } from 'lucide-react-native';
...
export default function ExerciseDetailScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const [currentLogs, lastLogs, siblings, maybePartner, hist] = await Promise.all([
      getSetLogsForSessionExercise(sid, exerciseId),
      getLastCompletedSetsForExercise(exerciseId, sid),
      getExercisesByDay(ex.day),
      ex.superset_partner_id ? getExercise(ex.superset_partner_id) : Promise.resolve(null),
  ...
  const rowToPatch = (r: Row, isDrop: boolean) => {
    ...
    return {
      ...
      drop_weight_lb: isDrop && Number.isFinite(dropWeight as number) ? dropWeight : null,
      drop_reps: isDrop && Number.isFinite(dropReps as number) ? dropReps : null,
        ...
        >
          ...
          >
            <Card style={styles.beatCard}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.beatValue}>{beatThis ?? 'No previous data'}</Text>
                {history.length > 0 ? (
              ...
              <HistorySparkline
                data={history
                  ...
                  .map((h) => ({
                    date: h.date,
                    score: exercise?.type === 'bodyweight'
                      ? h.best_reps
          ...
          {exercise?.type === 'superset' && partner ? (
            <Pressable
              ...
              style={({ pressed }) => [styles.partnerPill, pressed && { opacity: 0.7 }]}
            >
          ...
          <View style={styles.tableHeader}>
            ...
            <Text style={[styles.headCell, { flex: 1 }]}>Reps</Text>
            <Text style={[styles.headCell, { width: 32, textAlign: 'center' }]}>✓</Text>
          </View>
          ...
          <Card padded={false}>
            {rows.map((r, idx) => {
              ...
              return (
                <View
                  key={r.setNumber}
                  style={!isLast && styles.rowDivider}
                >
                  <View style={styles.tableRow}>
...
function EditExerciseSheet({
  ...
  const [partnerId, setPartnerId] = useState<number | null>(exercise.superset_partner_id);
      ...
      <View style={styles.sheetBackdrop}>
        ...
        >
          <View style={styles.sheet}>
                      ...
                      >
                        <Text
                          style={[
                            ...
                            partnerId === c.id && styles.partnerOptionTextActive,
                          ]}
                          numberOfLines={1}
                        >
                          {c.name}
            ...
            <Text style={styles.sheetHint}>
              Changes apply to this day in future sessions. Logged history stays linked to this exercise.
            </Text>
            ...
            >
              ...
              <Text style={styles.duplicateBtnText}>Duplicate as new entry</Text>
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  partnerOptionDay: { fontSize: 11, color: colors.textMuted, fontWeight: '500' },
});
